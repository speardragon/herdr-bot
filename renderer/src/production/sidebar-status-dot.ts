...
export type SidebarDotTone = "idle" | "working" | "done" | "blocked" | "unread";
export type SidebarDotLayout = "expanded" | "collapsed" | "pinned";

const STATUS_TONES: ReadonlySet<string> = new Set(["idle", "working", "done", "blocked"]);

/**
 * The runtime-status half of the dot: one of the four herdr states, or `null` when there is nothing
 * trustworthy to show -- "unknown"/"offline"/missing, or a disconnected transport (the caller's
 * separate stale-UI path, e.g. `isHostReachable`, renders that case).
 */

...
import type { ConversationTranscriptEntry, ResolveNoticeTargetAvatar, TranscriptComputerHandoff, TranscriptMessage, TranscriptPrompt, TranscriptThinking, TranscriptToolCall } from "./model";
...
export function ConversationTranscript({ entries, hasOlder = false, isLoadingOlder = false, loadOlder, isAgentRunning = false, renderComputerHandoff, renderPromptEntry, isTransportDown = false, isReadOnly = false, onCancelQueuedSend, onCopyMessage, onDeleteFailedSend, onReply, onStartThread, renderMessageReactionActions, renderMessageReactionPills, resolveTranscriptCardInteractions, onResendFailedSend, resolveAttachmentMedia, readAttachmentBytes, downloadAttachment, resolveReplyPreview, resolveMentionIdentity = () => null, resolveNoticeTargetAvatar, isReplyTargetInScope, onOpenReply, onOpenAutomation, localToolPermissionStore, resolveLocalToolPermission, transcriptCards, urlCards, threadRootId = null, transcriptHandleRef }: { entries: readonly ConversationTranscriptEntry[]; isAgentRunning?: boolean; isReadOnly?: boolean; renderComputerHandoff?(entry: TranscriptComputerHandoff): ReactNode; /** herdr-bot: a blocked bot's approval/question card (entry kind "prompt"). */ renderPromptEntry?(entry: TranscriptPrompt): ReactNode; resolveAttachmentMedia?: (source: string) => Promise<AttachmentMedia | null>; readAttachmentBytes?: (path: string, maxBytes: number) => Promise<AttachmentBytesResult | null>; downloadAttachment?: (path: string, suggestedName?: string) => Promise<boolean>; resolveReplyPreview?(targetId: string): TranscriptReplyPreview | null; resolveMentionIdentity?: ResolveMentionIdentity; /** herdr-bot (Task 9): the live roster avatar resolver for a `bot-message-sent` notice's target. */ resolveNoticeTargetAvatar?: ResolveNoticeTargetAvatar; isReplyTargetInScope?(targetId: string): boolean; localToolPermissionStore?: LocalToolPermissionStore; resolveLocalToolPermission?(input: ResolveLocalToolPermissionInput): Promise<unknown>; transcriptCards?: TranscriptCardRootMountContract; resolveTranscriptCardInteractions?: TranscriptCardInteractionContext; urlCards?: UrlCardProvider | null; threadRootId?: string | null; transcriptHandleRef?: { current: FindInChatTranscriptHandle | null } } & ConversationTranscriptActions) {
  ...
  return (
    <div aria-label={t("Conversation transcript")} aria-live="off" className="sand-virtual-transcript" ref={transcriptRef} role="log" tabIndex={0}>
      {entries.map((entry, index) => {
        ...
        if (entry.kind === "notice") return <TranscriptNoticeCard entry={entry} key={entry.id} resolveTargetAvatar={resolveNoticeTargetAvatar} />;
        if (entry.kind === "prompt") return renderPromptEntry?.(entry) ?? null;
